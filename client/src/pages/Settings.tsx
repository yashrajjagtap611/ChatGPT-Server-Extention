...
import { userAPI, getSessionInfo } from '../services/api';
...
const Settings: React.FC = () => {
  ...
  const loadSessionInfo = () => {
    ...
    if (session) {
      setSessionTimeout(session.user.sessionTimeout || 60);
    }
  };
  ...
  const handlePermissionChange = (website: string, hasAccess: boolean) => {
    setPermissions(prev => 
      prev.map(p => 
        p.website === website 
          ? { ...p, hasAccess, lastAccessed: new Date() }
          : p
      )
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await Promise.all([
        userAPI.updatePermissions(permissions),
        userAPI.updateSessionTimeout(sessionTimeout)
      ]);
