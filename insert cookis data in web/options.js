...
(function () {
  ...
  async function setCookie(cookie) {
    ...
    return new Promise((resolve, reject) => {
      chrome.cookies.set(details, (result) => {
        ...
        if (!result) return reject(new Error('Failed to set cookie'));
        resolve(result);
      });
    });
  }
  ...
  async function clearExistingCookies() {
    ...
    return new Promise((resolve, reject) => {
      chrome.cookies.getAll({ url }, async (cookies) => {
        ...
        log(`Cleared ${removed} existing cookies for chatgpt.com`);
        resolve(removed);
      });
    });
  }
  ...
  async function importCookies(cookies) {
    ...
    for (const c of cookies) {
      if (!validateCookie(c)) {
        skipped += 1;
        log(`Skipped invalid cookie: ${c && c.name ? c.name : JSON.stringify(c)}`);
        continue;
      }
      try {
        ...
        if (verified) {
          success += 1;
          log(`Imported: ${res.name} for ${res.domain || normalizeDomain(c.domain, c.name)}${res.path}`);
        } else {
          skipped += 1;
        ...
        skipped += 1;
        log(`Error importing ${c.name}: ${e.message}`);
      }
    }
  ...
  fileInput.addEventListener('change', async (e) => {
    ...
    try {
      ...
      if (!Array.isArray(data)) throw new Error('JSON must be an array of cookie objects');
      parsedCookies = data;
      importBtn.disabled = false;
      showStatus(`Loaded ${parsedCookies.length} cookies. Ready to import.`, '');
    ...
    loginBtn.addEventListener('click', async () => {
      try {
        ...
        try {
          ...
          if (who && who.user) loginStatus.textContent = `Logged in as ${who.user.email} (${who.user.role})`;
  ...
  if (oneClickBtn) {
    oneClickBtn.addEventListener('click', async () => {
      try {
        ...
        if (!j || !j.data || !Array.isArray(j.data)) throw new Error('bundle missing array data');
        parsedCookies = j.data;
        showStatus(`Loaded latest bundle '${j.name}' with ${parsedCookies.length} cookies. Importing...`, '');
        if (clearFirstEl.checked) await clearExistingCookies();
        const { success } = await importCookies(parsedCookies);
        showStatus(`Imported ${success} cookies. Opening ChatGPT...`, 'success');
        const url = 'https://chatgpt.com/';
